...
class PluginActor(
  ...
  val jarNameFile = jarName.replace(".jar", "")
  val client = new NvimAPI(
        ...
        case Received(data) =>
          ...
          if (channelId == 0) {
            try {
              ...
              val fut =
                client.command(s"let g:${jarNameFile}_channel=${channelId}")
            ...
            // TODO display err on Vim
            val msgId: Int = decoded.arr(1).value.asInstanceOf[Double].toInt
            val methodName: String = decoded.arr(2).value.toString()
            val argsIn: ArrayBuffer[Any] =
              decoded.arr(3).value.asInstanceOf[ArrayBuffer[Any]]
            logger.debug(
            ...
            try {
              ...
              // val res = Reflection.call("nvimhost.Foo$", "Heck", ujson.Num(10))
              logger.debug(
                  ...
                  for (item <- a.arr.map(_.value).toArray) {
                    if (item.isInstanceOf[Double])
                      ...
                      throw new RuntimeException(
                        s"Unsupported return type ${item.getClass.toString}, ujson.Arr return type only supports [Double, Int, Boolean or String]"
                      )
                  }
                  new ResponseMsg(
                    msgId,
              ...
              case e: Exception => {
                ...
                logger.error(err)
                connection ! Write(
                  ByteString(new ResponseMsg(msgId, upack.Str(err)).encode)
                )
              }
            }
          }
